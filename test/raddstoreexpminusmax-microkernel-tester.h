...
#define XNN_TEST_RADDSTOREEXPMINUSMAX_ELEMENT_EQ(ukernel, arch_flags, element_tile,elements_scale,  datatype, params_type,init_params)                               \
  TEST(ukernel, elements_eq_##element_tile)                                                                            \
  {                                                                                                                    \
    RAddStoreExpMinusMaxMicrokernelTester().elements(element_tile).Test(ukernel, init_params);                         \
  }

#define XNN_TEST_RADDSTOREEXPMINUSMAX_ELEMENT_DIV(ukernel, arch_flags, element_tile,elements_scale, datatype, params_type,init_params)               \
  TEST(ukernel, elements_div_##element_tile)                                                                           \
  {                                                                                           \
    if (elements_scale == "") {                                                                                         \
      for (size_t elements = element_tile * 2; elements < element_tile * 10; elements += element_tile) {               \
        RAddStoreExpMinusMaxMicrokernelTester().elements(elements).Test(ukernel, init_params);                         \
      }                                                                                                                \
    } else {                                                                                                           \
      for (size_t elements = element_tile * 2 * elements_scale;                                                         \
                  elements < element_tile * 10 * elements_scale;                                                        \
                  elements += element_tile * elements_scale) {                                                          \
        RAddStoreExpMinusMaxMicrokernelTester().elements(elements).Test(ukernel, init_params);                         \
      }                                                                                                                \
    }                                                                                                                  \
  }

#define XNN_TEST_RADDSTOREEXPMINUSMAX_ELEMENT_LT(ukernel, arch_flags, element_tile,  datatype, params_type,init_params)                \
...
#define XNN_TEST_RADDSTOREEXPMINUSMAX_ELEMENT_GT(ukernel, arch_flags, element_tile,  datatype, params_type,init_params)                \
