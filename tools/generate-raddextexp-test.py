...
def split_ukernel_name(name):
  match = re.fullmatch(r"xnn_(f16|f32)_raddextexp_ukernel__(.+)_u(\d+)(_acc(\d+))?", name)
  if match is None:
...
XNN_TEST_RADDEXTEXP_BATCH_EQ(ukernel,arch_flags, ${", ".join(TEST_ARGS)});  
XNN_TEST_RADDEXTEXP_BATCH_DIV(ukernel,arch_flags,  ${", ".join(TEST_ARGS)});
XNN_TEST_RADDEXTEXP_BATCH_LT(ukernel,arch_flags,  ${", ".join(TEST_ARGS)});
XNN_TEST_RADDEXTEXP_BATCH_GT(ukernel,arch_flags,  ${", ".join(TEST_ARGS)});
"""
